...
function setProgresSession(session) {
    ...
    try {
        const NP = window.Capacitor && window.Capacitor.Plugins && window.Capacitor.Plugins.NotifyPlugin;
        if (NP && typeof NP.saveMealSession === 'function' && session && session.uuid && session.token) {
            const r = NP.saveMealSession({ uuid: String(session.uuid), token: String(session.token) });
...
function progresLogout() {
    ...
    sessionStorage.removeItem(PROGRES_SESSION_KEY);
    clearProgresCache();
    // Progres session is gone: disarm the native auto-booker (it would
    // only fail auth) and drop its stored token.
    try {
        const NP = window.Capacitor && window.Capacitor.Plugins && window.Capacitor.Plugins.NotifyPlugin;
        if (NP) {
            if (typeof NP.cancelBooking === 'function') { const r = NP.cancelBooking(); if (r && typeof r.catch === 'function') r.catch(() => {}); }
            if (typeof NP.clearMealSession === 'function') { const r2 = NP.clearMealSession(); if (r2 && typeof r2.catch === 'function') r2.catch(() => {}); }
        }
    } catch (e) {}
    try { if (window.PortalNotify) window.PortalNotify.onLogout(); } catch (e) {}
    renderGradesSection();
...
function renderGradesSection() {
    ...
    if (session) {
        const gsn = document.getElementById('grades-student-name');
        if (gsn) gsn.textContent = 'نقاطي — ' + (session.name || '');
    } else {
        setGradesCardView(false);
        progresCurrentView = null;
        updateBnActive();
