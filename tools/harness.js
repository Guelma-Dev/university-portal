...
(async function () {
    ...
    check(!html.includes('sid-head"') && !html.includes('sid-titles'), 'no header chrome (card dominates)');
    check(html.includes('sid-flbl') && html.includes('sid-vlat') && !html.includes('sid-row'), 'stacked label rows + shared value rows (official structure)');
    check(html.includes('&nbsp;'), 'birth spacing like official');
    ...
    check(beManifest.versionCode === 12 && beManifest.versionName === '1.4.8', 'production manifest advertises 1.4.5 (code 9)');
    check(/^https:\/\/[^\/\s]+\/app\/releases\/app-12\.apk$/.test(beManifest.apkUrl), 'manifest apkUrl points at hosted release');
    check(fs.existsSync(path.join(ROOT, 'backend', 'flask_backend', 'releases', 'app-12.apk')), 'release APK present for hosting');
    ...
    check(cssLux.includes('19vh'), 'official screen top offset');
    check(cssLux.includes('4.7em'), 'official container radius');
    check(cssLux.includes('cubic-bezier(.45, 0, .55, 1)'), 'official default easing');
