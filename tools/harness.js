...
(async function () {
    ...
    check(beManifest.versionCode === 21 && beManifest.versionName === '1.4.14', 'production manifest advertises 1.4.14 (code 21)');
    check(/^https:\/\/[^\/\s]+\/app\/releases\/app-21\.apk$/.test(beManifest.apkUrl), 'manifest apkUrl points at hosted release');
    check(fs.existsSync(path.join(ROOT, 'backend', 'flask_backend', 'releases', 'app-21.apk')), 'release APK present for hosting');
