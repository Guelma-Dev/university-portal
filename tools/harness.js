...
(async function () {
    ...
    check(beManifest.versionCode === 18 && beManifest.versionName === '1.4.12', 'production manifest advertises 1.4.12 (code 18)');
    check(/^https:\/\/[^\/\s]+\/app\/releases\/app-18\.apk$/.test(beManifest.apkUrl), 'manifest apkUrl points at hosted release');
    check(fs.existsSync(path.join(ROOT, 'backend', 'flask_backend', 'releases', 'app-18.apk')), 'release APK present for hosting');
