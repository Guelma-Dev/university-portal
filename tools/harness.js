...
(async function () {
    ...
    check(beManifest.versionCode === 10 && beManifest.versionName === '1.4.6', 'production manifest advertises 1.4.5 (code 9)');
    check(/^https:\/\/[^\/\s]+\/app\/releases\/app-10\.apk$/.test(beManifest.apkUrl), 'manifest apkUrl points at hosted release');
    check(fs.existsSync(path.join(ROOT, 'backend', 'flask_backend', 'releases', 'app-10.apk')), 'release APK present for hosting');
