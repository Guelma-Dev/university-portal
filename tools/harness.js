...
(async function () {
    ...
    check(beManifest.versionCode === 17 && beManifest.versionName === '1.4.12', 'production manifest advertises 1.4.12 (code 17)');
    check(/^https:\/\/[^\/\s]+\/app\/releases\/app-17\.apk$/.test(beManifest.apkUrl), 'manifest apkUrl points at hosted release');
    check(fs.existsSync(path.join(ROOT, 'backend', 'flask_backend', 'releases', 'app-17.apk')), 'release APK present for hosting');
