...
(async function () {
    ...
    check(html.includes('sid-flbl') === false && html.includes('sid-row') && html.includes('sid-lat'), 'restored distributed rows (label|value, latin line)');
    ...
    check(beManifest.versionCode === 11 && beManifest.versionName === '1.4.7', 'production manifest advertises 1.4.5 (code 9)');
    check(/^https:\/\/[^\/\s]+\/app\/releases\/app-11\.apk$/.test(beManifest.apkUrl), 'manifest apkUrl points at hosted release');
    check(fs.existsSync(path.join(ROOT, 'backend', 'flask_backend', 'releases', 'app-11.apk')), 'release APK present for hosting');
    ...
    check(cssLux.includes('perspective: 1200px') === false, 'no perspective (official has none)');
