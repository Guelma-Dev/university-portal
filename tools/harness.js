...
(async function () {
    ...
    check(beManifest.versionCode === 20 && beManifest.versionName === '1.4.13', 'production manifest advertises 1.4.13 (code 20)');
    check(/^https:\/\/[^\/\s]+\/app\/releases\/app-20\.apk$/.test(beManifest.apkUrl), 'manifest apkUrl points at hosted release');
    check(fs.existsSync(path.join(ROOT, 'backend', 'flask_backend', 'releases', 'app-20.apk')), 'release APK present for hosting');
    check(fs.existsSync(path.join(ROOT, 'backend', 'flask_backend', 'releases', 'app-19.apk')), 'rollback APK (v19) retained');
