...
(async function () {
    ...
    check(beManifest.versionCode === 9 && beManifest.versionName === '1.4.5', 'production manifest advertises 1.4.5 (code 9)');
    check(/^https:\/\/[^\/\s]+\/app\/releases\/app-9\.apk$/.test(beManifest.apkUrl), 'manifest apkUrl points at hosted release');
    check(fs.existsSync(path.join(ROOT, 'backend', 'flask_backend', 'releases', 'app-9.apk')), 'release APK present for hosting');
